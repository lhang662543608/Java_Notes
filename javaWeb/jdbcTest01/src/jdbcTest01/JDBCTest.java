...
public class JDBCTest {
    ...
    public void testStatement() throws Exception {
        ...
        try {
            ...
            String sql = "insert into customers(name,email,birth) " +
                    "values ('xyz','xyz@163.com','1921-07-01')";
            //3.执行插入
            //3.1获取操作sql 语句的Statement对象
            statement = conn.createStatement();
    ...
    /**
     * 编写一个通用的方法，在不改变源程序的前提下，可以获取任何数据库的连接。
     * 解决方案：把数据库驱动Driver实现类的全类名、url、password放入一个配置文件中，通过修改配置文件的
     * 方式实现和具体数据库解耦。
     */
